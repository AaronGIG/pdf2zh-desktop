...
- 🐛 **修复 OCR 无视页码选择**（macOS）：勾了「OCR 识别」后，即使只选了「仅首页」或某几页，OCR 仍然把整本每一页都识别一遍（每页要好几秒，纯属白等）。现在 OCR 严格只处理选中的页码范围。
- 🔧 **扫描件译文残留英文减少**：扫描件翻译后常有部分正文没被翻译、留着原文英文，和中文交错在一起。原因是每行按各自识别框高度单独算字号，字号忽大忽小，触发了翻译引擎「字号偏小就当下标/公式、保留原文不翻」的判定。改成整页统一字号后，同页文字尺寸一致，正文被连续翻译，残留英文明显减少、可读性提升。
  - ⚠️ 说明：扫描件（纯图片）翻译受限于版面识别精度，效果仍不及可直接选中文字的数字版 PDF，边缘区域可能仍有少量未翻译原文。**有数字版时建议优先用数字版。**
