    ...
    def run(self):
        ...
        try:
            ...
            if self.ocr_mode:
                ...
                # v2.3.25: 管线抽到 pdf2zh/ocr_pipeline.py，Mac 与 Windows 共用一套实现
                from pdf2zh.ocr_pipeline import ScannedPdfTranslator
                import sys as _sys
                _asset = os.path.join(
                    getattr(_sys, "_MEIPASS", os.path.dirname(os.path.dirname(__file__))),
                    "assets")
                mono_path, dual_path = ScannedPdfTranslator(
                    file_path=self.file_path, output_dir=self.output_dir,
                    pages=self.pages, lang_out=self.lang_out,
                    translator=self._get_table_translator(), layout_model=model,
                    status_cb=self.status.emit,
                    progress_cb=lambda c, t: self.progress.emit(c, t),
                    is_cancelled=lambda: self.cancelled, asset_dir=_asset,
                ).translate()
                if self.cancelled:
