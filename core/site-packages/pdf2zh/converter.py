...
class TranslateConverter(PDFConverterEx):
    ...
    def receive_layout(self, ltpage: LTPage):
        ...
        ############################################################
        # v2.3.30: 识别左边距的行号，整页跳过（与 macOS 版同修）。
        #
        # 投稿稿/录用稿常在左边距逐行标行号（x≈35 的 '250' '251'…，字号与正文相同）。
        # 版面模型会把行号栏识别成 abandon → cls=0，而正文是 cls=2..6；两者在字符流里
        # 逐行交替出现，段落判断里的 `cls != xt_cls` 于是每行都成立 —— 整页退化成
        # **逐行翻译**。实测某录用稿 130 段、中位仅 83 字符，其中 17% 切在句子中间
        # （以 the/that/of 结尾），模型拿到半句只能编，译文里出现「适当的______」
        # 「与……相比」这类残缺句。跳过后：37 段、0% 残缺。
        #
        # 注意数字后面那个空格也要一起跳过——它同样落在行号栏的 abandon 区，
        # 只跳数字的话段落照样每行断一次（实测 130→99，改进有限）。
        # 代价是译文页不再渲染边距行号；双语对照的原文页仍保留，实测正文数字留存 100%。
        _skip_ids = set()
        try:
            if os.environ.get("P2Z_DISABLE_LINENUM_SKIP"):
                raise RuntimeError("disabled")
            _chars = [c for c in ltpage if isinstance(c, LTChar)]
            if len(_chars) > 200:          # 只对正文页做，图形块跳过
                ...
                for _row in _rows.values():
                    ...
                    _run = []
                    for _c in _row:
                        if _c.get_text().isdigit():
                            _run.append(_c)
                        else:
                            break
                    if not (1 <= len(_run) <= 5):
                        continue
                    if _run[0].x0 > _margin:
                        continue
                    _rest = _row[len(_run):]
                    _gap = None
                    for _c in _rest:
                    ...
                    if _gap is None or _gap < max(_run[-1].size, 6):
                        continue
                    for _c in _rest:                 # 连数字后的空白一起跳
                        if _c.get_text().strip():
                            break
                        _run.append(_c)
                    _cand.append(_run)
                if len(_cand) >= 5:                  # 要多行重复才认定是行号栏
                    for _run in _cand:
                        _skip_ids.update(id(c) for c in _run)
        except Exception:
            _skip_ids = set()
            ...
            if isinstance(child, LTChar):
                ...
                if not cur_v:
                    ...
                    if vbkt and child.get_text() == ")":
                        cur_v = True
                        vbkt -= 1
                # v2.3.10 fix (issue #28): 竖排表格常被版面检测模型识别成同一个 cls 区域，导致
                # 表格里相距很远、互不相关的多个旋转单词全部被当成"同一个公式"持续累积进 vstk。
                # 还原时每个公式字符的坐标只相对"本组第一个字符"计算偏移，再叠加同一个段落锚点，
                # 原本相隔很远的旋转单词就会被挤到几乎同一个位置，表现为文字被拆散、乱序堆叠。
                # 这里只在"公式字符紧接公式字符"时检查前后两个字符的间距，超过阈值就强制断开，
                # 不影响正常段落文字的换行/分段逻辑，也不影响间距本就很小的正常内联公式。
                far_v = (
                    bool(vstk)
                    and cur_v
                    and xt is not None
                ...
                if (                                                        # 判定当前公式是否结束
                    ...
                    or far_v                                                # 3. 与前一个公式字符间距过大（竖排表格里不相关的旋转文字）
                ...
                if not vstk:
                    if cls == xt_cls and not far_v: # 当前字符与前一个字符属于同一段落
        ...
        if vstk:    # 公式出栈
            ...
            varf.append(vfix)
        log.debug("\n==========[VSTACK]==========\n")
        for idx, v in enumerate(var):  # 计算公式宽度
            l = max([vch.x1 for vch in v]) - v[0].x0
            log.debug(f'< {l:.1f} {v[0].x0:.1f} {v[0].y0:.1f} {v[0].cid} {v[0].fontname} {len(varl[idx])} > v{idx} = {"".join([ch.get_text() for ch in v])}')
            vlen.append(l)
        ...
        def gen_op_txt_rot(font, size, a, b, c, d, x, y, rtxt):
            # v2.3.10 (issue #28): {vN} 公式/竖排字符原样保留时, gen_op_txt 之前一直
            # 硬编码单位矩阵(1 0 0 1), 只保留了字符的位置, 没保留旋转角度——横排公式/
            # 斜体角标看不出来, 但 90°/270° 旋转的竖排表格文字会被画成"正立字母摞成一列",
            # 而不是真正竖排。这里改用该字符自己原始的旋转子矩阵(matrix 的 a,b,c,d 分量
            # 按字号归一化), 只在 gen_op_txt_rot 这个专用分支里生效, 不影响原有 gen_op_txt
            # 调用的横排文字/公式渲染。
        ...
        for idx, new in enumerate(news):
            x: float = pstk[idx].x                       # 段落初始横坐标
            y: float = pstk[idx].y                       # 段落初始纵坐标
            x0: float = pstk[idx].x0                     # 段落左边界
            x1: float = pstk[idx].x1                     # 段落右边界
            height: float = pstk[idx].y1 - pstk[idx].y0   # 段落高度
            size: float = pstk[idx].size                 # 段落字体大小
            brk: bool = pstk[idx].brk                    # 段落换行标记
            cstk: str = ""                              # 当前文字栈
            ...
            fcur_ = fcur
            ptr = 0
            log.debug(f"< {y} {x} {x0} {x1} {size} {brk} > {sstk[idx]} | {new}")
            ...
            while ptr < len(new):
                ...
                if vy_regex:  # 加载公式
                    ...
                    if fcur_ == self.noto_name: # FIXME: change to CONST
                        # 当 noto 字体不可用时，使用默认宽度
                        if self.noto is not None:
                            adv = self.noto.char_lengths(ch, size)[0]
                        else:
                            # 使用估计的字符宽度（中文字符通常是 0.5 * size）
                            adv = 0.5 * size
                    else:
                ...
                if vy_regex:  # 插入公式
                    ...
                    for vch in var[vid]:  # 排版公式字符
                        vc = chr(vch.cid)
                        vrot = None
                        if vch.matrix[0] == 0 and vch.matrix[3] == 0 and vch.size:  # 竖排字符保留原始旋转角度
                            vrot = (
        ...
        # v2.3.3 fix: 译文块强制黑色填充+黑色描边。
        # 之前译文 BT 块不设颜色，继承外部图形状态；某些 PDF(如 Procedia/ScienceDirect
        # 期刊带红色页头)外部激活的是红色，导致整页译文渲染成红色。0 g 0 G 强制黑色兜底。
        color_reset = "0 g 0 G "
        if self.scan_mode:
            white_bg = ""
            for p in pstk:
