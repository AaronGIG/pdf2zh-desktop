...
class OnnxModel(DocLayoutModel):
    ...
    def predict(self, image, imgsz=1024, **kwargs):
        # v2.3.8: imgsz 只控制这里内部的推理分辨率(resize_and_pad_image 会自己 resize,
        # 输出框坐标经 scale_boxes() 换算回调用方传入的原始 image 尺寸, 不影响调用方那边的
        # 点↔像素坐标映射)。调用方对矮/小页面(如附录里嵌的小图表)算出来的 imgsz 可能远小于
        # 模型原生训练尺寸(该 onnx 模型文件名标注 imgsz1024), 导致 YOLO 候选框不足 300 个,
        # 触发 ONNX 里写死的 TopK(k=300) 报错("k argument [300] should not be greater than
        # specified axis dim value")。这里兜底锁下限, 不管调用方传多小都不会低于模型原生尺寸;
        # 调用方已经算出的正常/大页面 imgsz(本来就 ≥1024)不受影响。
