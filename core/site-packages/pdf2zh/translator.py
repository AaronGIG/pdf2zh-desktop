...
class DeepseekTranslator(OpenAITranslator):
    ...
    def prompt(self, text, prompt):
        # Mac: 用户没自定义 prompt 时使用强化版
        if prompt:
            return super().prompt(text, prompt)
        return [
            {"role": "system", "content": self._SYSTEM_PROMPT},
            {
                "role": "user",
                "content": (
                    f"Translate to {self.lang_out}. Apply rules above strictly.\n"
                    f"Source: {text}\n"
                    f"Translation:"
                ),
            },
    ...
    def prompt(self, text, prompt_template=None):
        """Override: 在 messages 前插入 system message 约束 DeepSeek 行为"""
