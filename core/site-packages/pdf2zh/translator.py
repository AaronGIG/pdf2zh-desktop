...
# 以下库延迟导入，避免启动时阻塞（ollama 无服务时卡死，azure/tencent 等加载慢）
# 在各 Translator 类的 __init__ 中按需导入
deepl = None
ollama = None
xinference_client = None
...
class DeepLTranslator(BaseTranslator):
    ...
    def __init__(
        ...
        self.client = _deepl.Translator(auth_key)
...
class OllamaTranslator(BaseTranslator):
    ...
    def __init__(
        ...
        self.client = _ollama.Client(host=self.envs["OLLAMA_HOST"])
...
class XinferenceTranslator(BaseTranslator):
    ...
    def __init__(
        ...
        import xinference_client as _xinference
        self.client = _xinference.RESTfulClient(self.envs["XINFERENCE_HOST"])
...
# v2.3.6: 清洗模型多输出的"非译文"(译者注/前言/思考块)。
# 弱模型或推理类模型常无视"只输出译文", 会加 (注：...)、"译文："前言、<think> 等。
_MT_NOTE_RE = re.compile(
    r"\s*[（(]\s*(注|註|译者注|譯者註|译注|譯注|Note|N\.?B\.?)\s*[：:].*$",
    re.DOTALL | re.IGNORECASE,
)
_MT_PREAMBLE_RE = re.compile(
    r"^\s*(译文|译|翻译|Translation|Translated text)\s*[：:]\s*", re.IGNORECASE
)
_MT_THINK_RE = re.compile(r"<think>.*?</think>", re.DOTALL | re.IGNORECASE)


def _strip_mt_artifacts(text):
    """去掉模型多加的译者注/前言/思考块; 只删带明确"注/Note"标记的结构, 不碰正文合法括号。"""
    if not text or not isinstance(text, str):
        return text
    orig = text
    text = _MT_THINK_RE.sub("", text)
    text = _MT_PREAMBLE_RE.sub("", text)
    text = _MT_NOTE_RE.sub("", text)
    text = text.strip()
    return text if text else orig.strip()  # 清洗后为空则回退, 避免整段清没
...
class OpenAITranslator(BaseTranslator):
    ...
    # v2.3.6: 所有 OpenAI 兼容服务(OpenAI/硅基/智谱/Azure/自定义...)通用强约束 system prompt,
    # 显式禁止模型加译者注/前言/解释。子类设了自己的 _SYSTEM_PROMPT(如 DeepSeek)就用它的。
    _SYSTEM_PROMPT = (
    ...
    def prompt(self, text, prompt_template=None):
        # v2.3.6: 在 base 用户消息前加强约束 system message(无 system 则加, 已有则不重复→避免子类双 system)
        messages = super().prompt(text, prompt_template)
        sysp = getattr(self, "_SYSTEM_PROMPT", None)
        if sysp and not (messages and messages[0].get("role") == "system"):
            return [{"role": "system", "content": sysp}] + list(messages)
        return messages
    ...
    def do_translate(self, text) -> str:
        ...
        # v2.3.5: content 可能为 None(思考模式偶发把结果全落 reasoning_content / 内容过滤) → 防 None.strip() 崩
        content = (response.choices[0].message.content or "").strip()
        content = self.think_filter_regex.sub("", content).strip()
        content = _strip_mt_artifacts(content)  # v2.3.6: 去模型多加的译者注/前言/思考块
        return content
...
class AzureTranslator(BaseTranslator):
    ...
    def __init__(
        ...
        self.client = TextTranslationClient(
            endpoint=endpoint, credential=_credential, region="chinaeast2"
        )
...
class TencentTranslator(BaseTranslator):
    ...
    def __init__(
        ...
        try:
            cred = tc_credential.DefaultCredentialProvider().get_credential()
...
class DeepseekTranslator(OpenAITranslator):
    ...
    envs = {
        ...
        "DEEPSEEK_MODEL": "deepseek-v4-flash",  # v2.3.5: deepseek-chat 别名 2026-07-24 退役, 改用显式非思考 v4 版本
    }
    CustomPrompt = True

    # Mac: 强化 system prompt - 保留 ASCII 数字/字母/算符/占位符不变
    _SYSTEM_PROMPT = (
        "You are a professional translation engine for academic and technical documents.\n"
        "STRICT RULES (must follow):\n"
        "1. Output ONLY the translated text - no explanations, comments, notes, or annotations.\n"
        "2. Preserve EXACTLY (do NOT translate or modify):\n"
        "   - All ASCII letters and digits (a-z A-Z 0-9)\n"
        "   - All math operators and symbols (+ - = < > ( ) [ ] | / * ^ _ ~ ! , . ' \" ; :)\n"
        "   - All formula placeholders {v0} {v1} {v2} ...\n"
        "   - All Greek letters (α β γ ... χ κ λ ...)\n"
        "   - All math symbols (≥ ≦ ≪ ≫ → ← ⇒ ⇔ ′ ″ ± × ÷ ∂ ∇ ∞ ∫ ∑ ∏ √)\n"
        "3. Only translate natural language words (Japanese kana/kanji, English prose, etc.)\n"
        "4. If the input is mostly formulas/numbers/operators with little prose, output it unchanged.\n"
    )
    ...
    def __init__(
        ...
        self.prompttext = prompt
        # Mac: 让强化版 system prompt 影响 cache key
        self.add_cache_impact_parameters("mac_strict_prompt", "v1")
        # v2.3.5: DeepSeek V4(v4-flash/v4-pro, 及已映射到 v4 的 deepseek-chat 别名)是混合推理模型,
        # 裸调默认走"思考模式" → 逐段先推理再输出, 导致翻译极慢/偶发漏译/(经代理时)推理泄漏进译文。
        # 对这些模型注入 thinking:disabled 退回直出(=旧 deepseek-chat 行为); deepseek-reasoner 是纯推理, 不动。
        _m = (self.model or "").lower()
        # v2.3.33: DeepSeek 除 deepseek-reasoner(纯推理,尊重用户显式选择)外一律关思考。
        # 原来只认 "v4"/"chat"，但官方新名 deepseek-flash 两个都不含，会漏掉、默认走思考
        # 模式（实测裸调吐 429 字推理）→ 翻译又慢又可能漏译。实测各模型加 disabled 都正常直出。
    ...
    # v2.3.6: 移除 DeepSeek 自己的 prompt override, 改用继承的 OpenAITranslator.prompt
    # (它会用 DeepseekTranslator._SYSTEM_PROMPT), 避免双 system message; 强约束逻辑统一到父类。
